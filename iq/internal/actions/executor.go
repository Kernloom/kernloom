...
// ApplyRelationship applies an authorized relationship deny. Relationship
// enforcement is binary: block or nothing; if the resolution downgrades below
// block the call is skipped.
func (e *SourceActionExecutor) ApplyRelationship(
	pep adapterruntime.RelationshipPEP,
	target adapterruntime.RelationshipTarget,
	r ActionResolution,
	now time.Time,
) ActionResult {
	result := ActionResult{
	...
	if r.ExecutableLevel != "block" {
		// Policy downgraded below block — relationship-level rate limiting is adapter-specific.
	...
	if pep == nil {
		result.Status = "skipped"
		result.Reason = "relationship pep unavailable"
		return result
	}
	if err := pep.DenyRelationship(target); err != nil {
		result.Status = "failed"
		result.Reason = err.Error()
		return result
	}
